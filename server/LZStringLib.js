...
const LZString = {
  ...
  decompressFromBase64 : (input) => {
    ...
    return LZString._decompress(input.length, 32, function(index) { return LZString.getBaseValue(keyStrBase64, input.charAt(index)); });
  },
  ...
  compressToEncodedURIComponent: (input) => {
    if (input == null) return "";
    return LZString._compress(input, 6, function(a) { return LZString.keyStrUriSafe.charAt(a);} );
  },
  ...
  decompressFromEncodedURIComponent: (input) => {
    ...
    return LZString._decompress(input.length, 32, function(index) { return LZString.getBaseValue(LZString.keyStrUriSafe, input.charAt(index)); });
  },
